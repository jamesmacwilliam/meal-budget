...
import { connection, query, create, find } from '../../src/server/db/queries/ingredients'
...
describe('ingredients', () => {
  ...
  it('creates new ingredients and queries for them', (done) => {
    async function Runner() {
      let ingredient = (await create('test'))[0]
      expect(await query('te')).not.to.be.empty
  ...
  it('excludes ingredients that do not match query', (done) => {
    async function Runner() {
      expect(await query('te')).to.be.empty
